...
;(function () {
  ...
  // ── Init conversation via REST ────────────────────────────────────────────
  async function initConversation() {
    try {
      const res = await fetch(`${API_URL}/api/conversations`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ tenantId: TENANT_ID, chatbotId: BOT_ID, channel: 'webchat', sessionId }),
      })
  ...
  async function connectWebSocket() {
    ...
    connection.onreconnecting(() => { statusDot.style.background = '#fbbf24'; statusTxt.textContent = 'Reconnexion...' })
    connection.onreconnected(async () => {
      statusDot.style.background = '#4ade80'; statusTxt.textContent = t.online
      if (conversationId) await connection.invoke('JoinConversation', conversationId)
    })
    connection.onclose(() => { statusDot.style.background = '#ef4444'; statusTxt.textContent = t.offline })

    try {
      await connection.start()
      statusTxt.textContent = t.online
      if (conversationId) await connection.invoke('JoinConversation', conversationId)
    } catch (err) {
      console.error('[BotFlow] WebSocket error:', err)
      statusDot.style.background = '#ef4444'
      statusTxt.textContent = t.offline
    } finally {
      isConnecting = false
    }
  }
